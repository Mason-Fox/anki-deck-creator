...
def _aiohttp_proxies_arg(proxy) -> Optional[str]:
    """Returns a value suitable for the 'proxies' argument to 'TestAddOn._vendor.aiohttp.ClientSession.request."""
    if proxy is None:
        return None
    elif isinstance(proxy, str):
        return proxy
...
async def parse_stream_async(rbody: TestAddOn._vendor.aiohttp.StreamReader):
...
class APIRequestor:
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_id: Optional[str] = None,
        request_timeout: Optional[Union[float, Tuple[float, float]]] = None,
    ) -> TestAddOn._vendor.aiohttp.ClientResponse:
        abs_url, headers, data = self._prepare_request_raw(
            url, supplied_headers, method, params, files, request_id
        )
    ...
    async def _interpret_async_response(
        self, result: TestAddOn._vendor.aiohttp.ClientResponse, stream: bool
...
@asynccontextmanager
async def aiohttp_session() -> AsyncIterator[TestAddOn._vendor.aiohttp.ClientSession]:
    user_set_session = openai.aiosession.get()
    if user_set_session:
        yield user_set_session
    else:
        async with TestAddOn._vendor.aiohttp.ClientSession() as session:
            yield session
